 ...
 *
 */
public class ComponentBrowseCache_SaveE255566
{
    ...
    */
   public ComponentBrowseCache_SaveE255566() {}
         ...
         List<String> cachedData )
   {
      ...
      synchronized (ComponentBrowseCache_SaveE255566.class)
      {
   ...
   /** 
    * Method to get the SREL release(s) cached. There should only be 1
    * SREL type entry (or 0) for the component, but handle the outlier
    * case where there may be several.
    * Returns a new List object separate from the cached data.
    * 
    * Abandoned this approach. Use Comp Browse Utils front door via
    * getValueSet which obtains cached data by attribute being
    * processed. This includes a RETAIN call to gather and cache data
    * on 1st reference. (That may also return error msgs as choices,
    ...
    */
   /*
   public static List<String> obtainCachedSRELRelease(
         String browseComponentValue )
   {
      // Attempt to find in cache 
      // Using component name as part of the key
      ...
      synchronized (ComponentBrowseCache.class)
      {
         ...
         if (cachedReleases != null && !cachedReleases.isEmpty())
         {
            ...
            // purposes
            result = new ArrayList<String>();
            for (String releaseValue : cachedReleases)
            {
               if (ComponentBrowseValueSetUtils.srelRelease( releaseValue ))
