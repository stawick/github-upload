 ...
 *
 */
public class ComponentBrowseCache_saveE275622
{
    ...
    */
   public ComponentBrowseCache_saveE275622() {}
         ...
         List<String> cachedData )
   {
      ...
      synchronized (ComponentBrowseCache_saveE275622.class)
      {
         ...
         SportCommonData sportCommonData )
   {
      ...
      if (projectAreaComponents == null || projectAreaComponents.isEmpty())
      {
         ...
         /*
         // _F275622A Add (unique) components from existing APARs.
         List<String> aparComponents = obtainExistingWorkItemComponents(
               AparConstants.WORK_ITEM_TYPE_ID,
               AparConstants.COMPONENT_ATTRIBUTE_ID, projectAreaComponents,
               sportCommonData );
         // Practice safety with null lists.
         if (aparComponents != null && !aparComponents.isEmpty())
         {
            if (projectAreaComponents != null)
            {
               projectAreaComponents.addAll( aparComponents );
            }
            else
            {
               projectAreaComponents = aparComponents;
            }
         }
         */
         // Set in cache
   ...
   // _F275622A
   /**
    * After a server restart, gather component selections from existing
    * work items in the project area. If project area customization
    * values already handled, do not redundantly process those.
    * 
    * @param workItemType - Which work items to query.
    * @param componentAttributeId - Specific to work item type.
    * @param projectAreaComponents - Components already handled,
    *        do not redundantly process.
    * @param sportCommonData
    * @return
    */
   protected static List<String> obtainExistingWorkItemComponents(
         String workItemType, String componentAttributeId,
         List<String> projectAreaComponents,
         SportCommonData sportCommonData )
   {
      List<String> componentValues = null;
      try
      {
         ...
         */
         /*
         ITeamRepository repository = sportCommonData.getProjectArea()
               .getOrigin();
         */
         /*
         sportCommonData.setQueryCommon( (IQueryCommon)repository
               .getClientLibrary( IQueryCommon.class ) );
         */
         // ITeamRepository repository = projectArea.getOrigin();
         // queryCommon = (IQueryCommon)projectArea.getOrigin().
         ...
         /*
         IQueryableAttribute aparNumAttribute = RtcUtils
               .findQueryableAttribute( AparConstants.APAR_NUM_ATTRIBUTE_ID,
                     sportCommonData );
         */
         ...
         IQueryableAttribute workItemTypeAttribute = RtcUtils
               .findQueryableAttribute( IWorkItem.TYPE_PROPERTY,
                     sportCommonData );
         AttributeExpression wiTypeExpression = new AttributeExpression( workItemTypeAttribute,
               AttributeOperation.EQUALS, workItemType );
         ...
         */
          ...
          * Component != ...
          */
         // Do NOT include components already obtained by project area
         // customization.
         if (projectAreaComponents != null)
         {
            IQueryableAttribute componentAttribute = RtcUtils
                  .findQueryableAttribute( componentAttributeId,
                        sportCommonData );
            for (String projAreaCompValue : projectAreaComponents)
            {
               // Due to the != term(s), we need to perform our own
               // query here, below. Cannot reuse RtcUtils method.
               AttributeExpression compExpression = new AttributeExpression(
                     componentAttribute, AttributeOperation.NOT_EQUALS,
                     projAreaCompValue );
               term.add( compExpression );
            }
         }
          ...
          */
         /*
         IQueryResult<IResolvedResult<IWorkItem>> queryResults = RtcUtils
               .queryWorkItemsByAttributes( attributeValues,
                     sportCommonData );
         */
         // !!!!! queryCommon is NULL from Cbrowse VS Utils!!!!!
         IQueryResult<IResolvedResult<IWorkItem>> queryResults = queryCommon
          ...
          * Query results -
          */
         // Results should NOT have components already processed
         // (see projectAreaComponents parameter).
         while (queryResults.hasNext( monitor ))
         {
            IResolvedResult<IWorkItem> resolved = queryResults.next( monitor );
            IWorkItem workItem = resolved.getItem();
            String wiComponentValue = RtcUtils.getAttributeValueAsString(
                  workItem, componentAttributeId, sportCommonData );
            if (wiComponentValue != null && wiComponentValue.length() > 0 )
            {
               // Only add specific component once
               if (componentValues == null || !componentValues
                     .contains( wiComponentValue.toUpperCase() ))
               {
                  if (componentValues == null)
                  {
                     // Adding 1st entry
                  ...
                  componentValues.add( wiComponentValue.toUpperCase() );
                  // Neither release nor change teams defined yet,
                  // initialize in map to null
                  boolean firstReference = false;
                  synchronized (ComponentBrowseCache_saveE275622.class)
                  {
                     firstReference = (!(componentCache.containsKey(
                           constructReleaseCacheKey( wiComponentValue ) ))
                           && !(componentCache
                                 .containsKey( constructChangeTeamCacheKey(
                                       wiComponentValue ) )));
                  }
                  if (firstReference)
                  {
                     clearComponentData( wiComponentValue );
                  }
               }
            }
         }
      ...
      catch (Exception e)
      {
         ...
         Log log = sportCommonData.getLog();
         String errMsg = "SPoRT Component Browse Cache error encountered. "
            + "Problem encountered obtaining existing APARs "
            + "for initial project area components. "
            + "No components returned."
            + Constants.NEWLINE
            + e.getMessage();
