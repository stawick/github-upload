...
public class AparUtils_saveT266500
{
   ...
   static
   {
      Thread thread = new Thread( new Runnable()
      {
         public void run()
         {
            Log log = LogFactory.getLog( AparUtils_saveT266500.class );
            while (true)
            {
               try
               {
         ...
         throws TeamRepositoryException
   {
      ...
      if (tempFix.length() > 0)
         if (tempFix.contains( "* HIPER *" ))
            tempFix.substring( 35 );
      tempFix = hiperBanner.concat( tempFix );
      IAttribute tempFixAttribute = RtcUtils.findAttribute( apar,
            AparConstants.DRAFT_TEMPORARY_FIX_ATTRIBUTE_ID, sportCommonData );
      RtcUtils.setAttributeValue( apar, tempFixAttribute, tempFix,
            sportCommonData );
   }
         ...
         throws SportUserActionFailException, TeamRepositoryException
   {
      ...
      if (!messageToSubmitter.contains( newMessage ))
         newMessage.concat( messageToSubmitter );
      IAttribute messageToSubmitterAttribute = RtcUtils.findAttribute( apar,
            AparConstants.DRAFT_MESSAGE_TO_SUBMITTER_ATTRIBUTE_ID,
            sportCommonData );
      RtcUtils.setAttributeValue( apar, messageToSubmitterAttribute,
         ...
         throws TeamRepositoryException
   {
            ...
            .keySet())
      {
         ...
         // _T266500? TBD - We kinda *can* copy a stringList to/from
         // a string of a proper format... (Relax the exception for
         // that?). Or just change the related "store" attribute to
         // also be a stringList and do straight object value copying.
         // Dont kluge it up.
               ...
               .put( AparConstants.APPLICABLE_COMPONENT_LVLS_ATTRIBUTE_ID,
                     RtcUtils
                           .getUniqueValues(
                                 ...
                                 // temp fix for now -
                                 AparConstants.ATTRIBUTE_ID_PREFIX
                                 + "draftApplicableComponentLvls1", // _T266500C
      ...
      for (String draftAttr : extractedValues.keySet())
      {
         ...
         if (draftAttribute != null)
         {
            ...
            else
            {
               ...
               // _T266500? TBD - This may be able to handle string
               // to stringList attributes?
               RtcUtils.setAttributeValue( aparToUpdate, draftAttribute,
